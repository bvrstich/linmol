...
/**
 * fill the (*this) object with the Hamiltonian of the atom + the mean field generated by the other core
 * @param si the input SphInt object containing the spherical matrixelements
 */
void TPM::atom_mf(const SphInt &si){
   ...
   for(int B = 0;B < gnr();++B){
      ...
      for(int i = 0;i < gdim(B);++i){
         ...
         for(int j = i;j < gdim(B);++j){
            ...
            if(b == d)
               (*this)(B,i,j) +=  1.0/(N - 1.0) * (si.gT(a_me,c_me) + si.gU(a_me,c_me) + si.gh(a_me,c_me));

            if(a == d)
               (*this)(B,i,j) +=  sign /(N - 1.0) * (si.gT(b_me,c_me) + si.gU(b_me,c_me) + si.gh(b_me,c_me));

            if(b == c)
               (*this)(B,i,j) +=  sign /(N - 1.0) * (si.gT(a_me,d_me) + si.gU(a_me,d_me) + si.gh(a_me,d_me));

            if(a == c)
               (*this)(B,i,j) +=  1.0/(N - 1.0) * (si.gT(b_me,d_me) + si.gU(b_me,d_me) + si.gh(b_me,d_me));
