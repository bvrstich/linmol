...
void SubSys::construct_mf(const TPM &tpm, const SphInt &si_full){
   ...
   //fill the h-Matrix with the deorthogonalized mean-field
   h->L_map(*L,mf);

}

/**
 * construct the subsystem mean-field, generated by a point charge at the distance of the other core
 * @param tpm input TPM defining the full system
 */
void SubSys::construct_pc(const TPM &tpm){

   double charge = N - this->subocc_func(tpm);

   charge =  CartInt::gZ( (core + 1)%2 ) - charge;

   *h = 0.0;

   h->daxpy(charge/(double)CartInt::gZ( (core + 1)%2), si->gU((core + 1)%2) );
